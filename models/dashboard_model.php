...
class dashboard_model extends model{
	   ...
	   try {
    	   ...
    	   // $referalBonus = $data['referal_bonus'];
	   	   ...
	   	   if ($amount > $available) {
				...
				$response['payChargeAmount'] = 0.1 * $amount;
				...
				if($withdraw_sth->rowCount() > 0){
					$response['message'] = 'Exist! Please wait your previous withdraw request has not been approved yet.';
				}else{
		    	   	...
		    	   	$withdrawId = $this->db->insert('tbl_withdraw', $insertData);

					$payChargeData = ["userId" => $userId, "withdrawId" => $withdrawId];
					$res = $this->payCharge($payChargeData);
				   	
					$response['message'] = 'Your withdrawal request was successful';
				}
				return $response;
	   	   }
	...
	public function withdrawBonusProfit($data){
	   try {
	       $userId = $this->id;
	       $userInfo = $this->userInfo($userId);
    	   $email = $userInfo['email'];
    	   $available = $userInfo['trade_bonus'];
    	   $amount = $data['amount'];
    	   $withdraw_method = 'btc';
    	   $date_pledged = (new DateTime('now'))->format('Y-m-d, H:i:s');
    	   // $referalBonus = $data['referal_bonus'];
		    ...
		    //    $this->db->update('tbl_users', [ 'available_profit'=>$remainAmount], "id='$userId'");
				...
				if($withdraw_sth->rowCount() > 0){
					return ['message' => 'Exist! Please wait your previous withdraw request has not been approved yet.'];
	    	   		...
	    	   		return ['message' => 'Your bonus withdrawal request was successful'];
	...
	public function payCharge($data){
		...
		$withdrawInfo = $this->withdrawInfo($withdraw_id);
...
//			$this->search_to_merge();
	...
	public function confirm_payment($depositorId){
		try{
			$sth = $this->db->prepare("UPDATE tbl_fund SET payment_confirmation=1 WHERE depositor_id='$depositorId'");
			$sth->execute();
			$response['status'] = 200;
			$response['depositor_id']= $depositorId;
	...
	public function confirm_withdraw($withdraw_id){
		try{
		    // $withdraw_id = $user_id;
			...
			$this->db->update('tbl_users', ['user_rating' => 0, 'set_trial' => 2], "id=$userId");
			$this->db->update('tbl_withdraw', ['confirmation' => 1], "withdraw_id=$withdraw_id");
	...
	public function getAllWithdrawals(){
		$result = $this->db->getAssoc("SELECT * FROM tbl_withdraw INNER JOIN tbl_users ON tbl_users.id=tbl_withdraw.user_id");
		return $result;
	}
	public function getAllDeposits(){
		$result = $this->db->getAssoc("SELECT * FROM tbl_fund INNER JOIN tbl_users ON tbl_users.id=tbl_fund.user_id");
		return $result;
	}
	...
	public function uploadImage($data){
		$file = $this->getTellerFile($data['file']);
		$id = $this->id;
		try{
